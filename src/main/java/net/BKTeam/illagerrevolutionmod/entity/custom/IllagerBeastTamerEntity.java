...
public class IllagerBeastTamerEntity extends SpellcasterIllager implements GeoEntity, InventoryCarrier, RangedAttackMob {
    ...
    private   <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        ...
        if (event.isMoving() && !this.isAggressive() && !this.isCastingSpell()) {
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.beasttamerillager.walk"));
        }
        else if (this.isCastingSpell()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.beasttamerillager.summon"));
        }

        else if (this.isWieldingTwoHandedWeapon() && event.isMoving()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.beasttamerillager.walk2"));
        }
        else if (this.isWieldingTwoHandedWeapon() && !event.isMoving() && isAggressive()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.beasttamerillager.attack1"));

        }
        else event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.beasttamerillager.idle"));
