...
public class Blade_KnightEntity extends SpellcasterKnight implements GeoEntity, InventoryCarrier{
    ...
    private   <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        ...
        if (event.isMoving() && !this.isAggressive() && !this.isAttacking() && !this.isCastingSpell() && !this.isStartAnimationLowHealth()) {
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.IllagerBladeKnight.walk" + s2));

        }else if (this.isStartAnimationLowHealth()) {
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.IllagerBladeKnight.lowhealth"));

        }else if (event.isMoving() && this.isAggressive() && !this.isAttacking()) {
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.IllagerBladeKnight.walkhostile"+s2));
        }
        else if (this.isAttacking()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.IllagerBladeKnight.attack"+s1));
        }
        else if (this.isCastingSpell()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.IllagerBladeKnight.summon1"));
        }
        else event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.IllagerBladeKnight.idle"+s2));
        ...
        if(this.isStartAnimationLowHealth()){
            ...
            if(this.lowHealtTimer<=39 && this.lowHealtTimer>=20){
                if(this.lowHealtTimer==39){
                    ...
                    for (int i=0;i<6;i++){
                        Vec3 pos=new Vec3(this.getX()+this.level.getRandom().nextInt(-3,3),this.getY()+3.0D,this.getZ()+this.level.getRandom().nextInt(-3,3));
    ...
    public void die(DamageSource pCause) {
        this.knights.forEach(knight->{
            ...
            knight.hurt(this.damageSources().magic(),knight.getMaxHealth());
        });
        super.die(pCause);
    }
        ...
        protected void performSpellCasting() {
            if(Blade_KnightEntity.this.getTarget() instanceof Player)

                {
                    ...
                    level.playLocalSound(owner.getX(),owner.getY(),owner.getZ(),SoundEvents.AMBIENT_NETHER_WASTES_MOOD.get(),SoundSource.HOSTILE,5.0f,-5.0f,false);
