...
public class IllagerMinerBadlandsEntity extends AbstractIllager implements GeoEntity, InventoryCarrier {
    ...
    private   <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        if (event.isMoving() && !this.isAggressive() && !this.isAttacking() && !this.isAttackLantern()) {
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.illagerminerbadlands.walk"+(this.isHasItems() ? "3" : "")));
        }
        else if (this.isAttacking() && this.isAttackLantern()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.illagerminerbadlands.attack2"));
        }
        else if (this.isAttacking()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.illagerminerbadlands.attack"));
        }
        else if (this.isAggressive() && event.isMoving() && !this.isAttacking()){
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.illagerminerbadlands.walk2"));
        } else if (this.isSprinting() && event.isMoving()) {
            event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.illagerminerbadlands.walk3"));

        } else event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.illagerminerbadlands.idle"));
