...
public class Summoned_Soul extends ThrowableItemProjectile {

    int life;

    public Summoned_Soul(EntityType<? extends Summoned_Soul> type, Level world) {
        super(type,world);
        this.setNoGravity(true);
        this.life=0;
    }

    public Summoned_Soul(LivingEntity thrower, Level level) {
        super(ModEntityTypes.SUMMONED_SOUL.get(),thrower,level);
        this.setNoGravity(true);
        this.life=0;
    }
    public Summoned_Soul(Level world, double x, double y, double z) {
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if(!(result.getEntity() instanceof GlowItemFrame)){
            ...
            if(owner!=null){
                ...
                Soul_Entity soul=new Soul_Entity((LivingEntity) owner,owner.level, ModConstants.LIST_NAME_ZOMBIFIED.get(this.level.getRandom().nextInt(0,4)), (LivingEntity) owner,pos.getY());
                soul.setOwner(owner);
                soul.moveTo(pos,0.0f,0.0f);
                owner.level.addFreshEntity(soul);
                livingEntity.hurt(this.damageSources().generic(),4);
                this.discard();
            }
        }
    }
    ...
    protected void onHitBlock(BlockHitResult result) {
        ...
        BlockPos posX= new BlockPos(pos.getX(),pos.getY()+2,pos.getZ());
        if(this.getOwner()!=null){
            Soul_Entity soul=new Soul_Entity((LivingEntity) this.getOwner(),this.getOwner().level, ModConstants.LIST_NAME_ZOMBIFIED.get(this.level.getRandom().nextInt(0,4)), (LivingEntity) this.getOwner(),posX.getY());
